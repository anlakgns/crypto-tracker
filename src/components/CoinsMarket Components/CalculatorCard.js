...
export const CalculatorCard = ({responseCoins})=> {
  ...
  useEffect(()=> { // Data Fetching and Editting
    const fetchData = async () => { 
      ...
      const currencyNameListResponse = await openExchangeAPI.get("/currencies.json")
      ...
      Object.entries(currencyNameListResponse.data).map(currency => {
        return currencyListEditted.push({currencyName: currency[1], currencyCode: currency[0], currencyRate: rates[currency[0]] })
      })
  ...
  useEffect( ()=> { // Switch Logic for Calculator.
      if(selectedCoin !== "" && selectedCurrency !== "" && coinQuantity !== "" && tabValue === 1) {
        calculatorForFiat();
      }
  ...
  useEffect(()=> { // Cleanup for result
   ...
   const currencyFormatter = (num, currencyCode)=> {
    ...
    const numb = Number(num)    
      return new Intl.NumberFormat('en-EN', {style: 'currency', currency: currencyCode || "USD" }).format(numb)
  }
    ...
    <>
      <Grid container className={classes.calcContainer} >
        ...
        <Grid 
          item 
          xs={12} 
          className={classes.calcPriceInfoContainer}>
          {
            selectedCoin ? 
              <Typography 
                className={classes.calcPriceInfo} 
                align="center"> 
                1 {selectedCoin.name} ({selectedCoin.id}) = {currencyFormatter(selectedCoin.price)}
              </Typography> : 
            null
          } 
        </Grid>
